...
(function() {
  var View, XHR, ajax, app, currentTestId, defaultModel, expect, express, extend, port, syncXDR, testcount;
  ...
  if (!isNodeJs) {
    ...
    if (runTest) {
      ...
      ajax("//cors-test.appspot.com/test", "foo", function(err, result) {
        return expect(result, '{"status":"ok"}', "async ajax post");
      });
      ajax("/api/log", "foobar", function(err, result) {
