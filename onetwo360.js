...
(function() {
  var View, ajax, app, currentTestId, defaultModel, expect, express, extend, port, testcount;
  ...
  if (!isNodeJs) {
    ajax = function(url, data, cb) {
      ...
      if (!!cb) {
        xhr.onreadystatechange = function() {
          if (xhr.readyState === 4) {
            return cb((xhr.status === 200 ? null : xhr.status), xhr.responseText);
          }
        };
      }
      xhr.send(data);
    ...
    if (runTest) {
      ...
      ajax("http://cors-test.appspot.com/test", void 0, function(err, result) {
        return expect(result, '{"status":"ok"}', "async ajax");
      });
      expect(1, 1);
      ajax("http://cors-test.appspot.com/test", "foo", function(err, result) {
